...
class PostController extends Controller
{
    ...
    public function edit($locale, Post $post): View
    {
        ...
        foreach (array_keys(config('locales')) as $localeName) {
            foreach ($post->tags as $tag) {
                $translatedTag = $tag->getTranslations('name', [$localeName]);
                if (!empty($translatedTag)) {
                    $tags[$localeName][] = $translatedTag[$localeName];
                }
            }
        }
        ...
        return view('admin.posts.edit', [
            'post' => $post,
            'tags' => $tags,
            'users' => User::query()->authors()->pluck('name', 'id'),
            'media' => MediaLibrary::query()->first()->media()->get()->pluck('name', 'id'),
